...
fn build_gltf_json(
    ...
    for (mesh_i, mesh_info) in meshes.iter().enumerate() {
        ...
        for (offset, len) in [
            ...
            (mesh_info.tangents_offset, vertex_count * 16),
        ] {
        ...
        let tan_acc = pos_acc + 2;
        let tc0_acc = pos_acc + 3;
        let tc1_acc = pos_acc + 4;
        let col_acc = pos_acc + 5;
        let idx_acc = pos_acc + 6;
...
mod tests {
    ...
    fn test_gltf_primitives_split_by_submesh_material() {
        ...
        let views = gltf_data.json["bufferViews"].as_array().unwrap();
        let bv = accessors[second_indices]["bufferView"].as_u64().unwrap() as usize;
        // Accessor order: POSITION, NORMAL, TANGENT, TEXCOORD_0/1, COLOR_0, index.
        let shared_idx_bv = accessors[6]["bufferView"].as_u64().unwrap() as usize;
        assert_eq!(
            views[bv]["byteOffset"].as_u64().unwrap(),
            views[shared_idx_bv]["byteOffset"].as_u64().unwrap() + 12
        );
    }
    ...
    fn test_submeshes_split_into_primitives_with_materials() {
        ...
        let full_bv = accessors[6]["bufferView"].as_u64().unwrap() as usize;
