...
//!
//! ```c
//! GroveSpecies* species = grove_species_parse(toml_ptr, toml_len);
//! GroveTree* tree = grove_tree_generate(species, seed);
//! int status = grove_tree_export_glb(species, tree, path_ptr, path_len);
//! grove_tree_free(tree);
//! grove_species_free(species);
//! ```
//!
//! ## Safety
//!
...
pub type GroveSpeciesHandle = *mut Species;

/// Opaque handle to a generated tree.
pub type GroveTreeHandle = *mut Tree;
...
/// `grove_species_free`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grove_species_parse(
    toml_ptr: *const u8,
    toml_len: usize,
) -> GroveSpeciesHandle {
...
/// # Safety
///
/// The handle must have been returned by `grove_species_parse` and must not be
/// used after this call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grove_species_free(handle: GroveSpeciesHandle) {
    if !handle.is_null() {
...
/// # Safety
///
/// `species` must be a valid handle returned by `grove_species_parse`.
/// The returned tree borrows nothing from the species and must be freed with
/// `grove_tree_free`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grove_tree_generate(
...
/// # Safety
///
/// `tree` must be a valid handle returned by `grove_tree_generate`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grove_tree_stem_count(tree: GroveTreeHandle) -> u32 {
    if tree.is_null() {
        return 0;
    }
...
/// `tree` must be a valid handle returned by `grove_tree_generate`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grove_tree_leaf_count(tree: GroveTreeHandle) -> u32 {
    if tree.is_null() {
        return 0;
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grove_tree_export_glb(
    species: GroveSpeciesHandle,
...
/// # Safety
///
/// The handle must have been returned by `grove_tree_generate` and must not be
/// used after this call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grove_tree_free(handle: GroveTreeHandle) {
    if !handle.is_null() {
        drop(unsafe { Box::from_raw(handle) });
    }
}
...
"#;
    ...
    fn parse_generate_export_roundtrip() {
        unsafe {
            let species = grove_species_parse(OAK_TOML.as_ptr(), OAK_TOML.len());
            assert!(!species.is_null());

            let tree = grove_tree_generate(species, 42);
            assert!(!tree.is_null());
            assert!(grove_tree_stem_count(tree) > 0);

            let dir = tempfile::tempdir().unwrap();
            let path = dir.path().join("ffi_tree.glb");
            let path_str = path.to_str().unwrap();
            let status = grove_tree_export_glb(species, tree, path_str.as_ptr(), path_str.len());
            assert_eq!(status, GroveStatus::Ok as c_int);
    ...
    fn invalid_arguments_are_rejected() {
        unsafe {
            assert!(grove_species_parse(std::ptr::null(), 0).is_null());
            assert!(grove_species_parse(b"not toml".as_ptr(), 8).is_null());
            assert!(grove_tree_generate(std::ptr::null_mut(), 1).is_null());
            assert_eq!(grove_tree_stem_count(std::ptr::null_mut()), 0);
            assert_eq!(
                grove_tree_export_glb(std::ptr::null_mut(), std::ptr::null_mut(), b"x".as_ptr(), 1),
                GroveStatus::InvalidArgument as c_int
            );
            grove_tree_free(std::ptr::null_mut());
            grove_species_free(std::ptr::null_mut());
        }
    }

    #[test]
    fn version_is_nul_terminated() {
        let version = grove_version();
